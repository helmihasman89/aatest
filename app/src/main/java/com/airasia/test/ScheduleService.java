...
import static com.airasia.test.exception.ScheduleException.SCHEDULE_GENERAL_EXCEPTION;
import static com.airasia.test.exception.ScheduleException.SCHEDULE_INVALID_EXCEPTION;
import static com.airasia.test.settings.Constant.ENGINEER_OFF_DAYS;
import static com.airasia.test.settings.Constant.MAX_SHIFTS_PER_DAY;
import static com.airasia.test.settings.Constant.MAX_SHIFTS_PER_ENGINEER;
import static com.airasia.test.settings.Constant.SCHEDULE_PERIOD_DAYS;
...
public class ScheduleService {
    ...
    // Invalid engineers who are not eligible to enter the pool.
    @VisibleForTesting
    List<String> removedEngineers = new ArrayList<>();
    private int takenShifts;
    ...
    public List<Schedule> getSchedules() {
        ...
        System.out.println("From inside scheduleService");
//        for(Schedule currentX : schedules) {
    ...
    Engineer getRandomEngineer() {
        if (engineers != null) {
            if (engineers.size() > 0) {
                ...
                // Making sure that after takenShifts >= totalShifts / MAX_SHIFTS_PER_DAY, find each engineer
                // who dose not have any shift assigned already and then randomly select one.
                if (takenShifts >= totalShifts / MAX_SHIFTS_PER_DAY * ENGINEER_OFF_DAYS) {
